...
export const openPushStream = (filename: string) => {
    ...
    let wavFileHeader = readWavFileHeader(filename);
    ...
    let fmt: sdk.AudioStreamFormat = sdk.AudioStreamFormat.getWaveFormat(wavFileHeader.framerate, wavFileHeader.bitsPerSample, wavFileHeader.nChannels, format);

    // create the push stream we need for the speech sdk.
    let pushStream = sdk.AudioInputStream.createPushStream(fmt);
    ...
    fs.createReadStream(filename, { start: 44 }).on('data', function (arrayBuffer) {
        pushStream.write(arrayBuffer.slice() as ArrayBuffer);
    }).on('end', function () {
        pushStream.close();
    });
...
export const readWavFileHeader = (audioFileName: string) => {
    let fd = fs.openSync(audioFileName, 'r');
    ...
    // Format size
    let formatSize = ReadInt32(fd);
    if (formatSize > 16) {
        throw "Error reading .wav file header. Expected format size 16 bytes. Actual size: " + String(formatSize);
    }
    // Format tag
    let tag = ReadUInt16(fd);
    let nChannels = ReadUInt16(fd);
    let framerate = ReadUInt32(fd);
    // Average bytes per second
    ReadUInt32(fd);
    // Block align
    ReadUInt16(fd);
    let bitsPerSample = ReadUInt16(fd);
